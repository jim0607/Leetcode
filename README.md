...
solution 1: 利用对称性质: x,y=abs(x),abs(y); append neighbor to q only if (neighbor_x >= -2 and neighbor_y >= -2) <br>
solution 2!!!: 从source和destination两端同时进行bfs!!!! if visited_src & visited_des: return cnt_src + cnt_des <br>
solution 3: dp: cache[(x, y)] = min(dp(abs(x-1), abs(y-2)), dp(abs(x-2), abs(y-1))) + 1 <br>
