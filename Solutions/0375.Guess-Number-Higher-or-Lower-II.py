...
class Solution:
    def getMoneyAmount(self, n: int) -> int:
        ...
        for i in range(n - 2, 0, -1):       # 要逆序遍历，在这里debug了5min
            for j in range(i + 2, n + 1):
                mid = i + (j - i + 1) // 2  # 这里是错的，在这里debug了15min
...
class Solution:
    def getMoneyAmount(self, n: int) -> int:
        ...
        for i in range(n - 2, 0, -1):
            for j in range(i + 2, n + 1):
                for mid in range(i + 1, j):     # mid可以是任意值而不仅仅是(i, j)的中间值eg: n = 10, mid is chosen to be 7.
