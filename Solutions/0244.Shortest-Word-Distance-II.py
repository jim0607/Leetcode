...
class WordDistance:
    ...
    def shortest(self, word1: str, word2: str) -> int:
        """
        直接去self.posDict里面去找就可以了，如果len(posDict[word1])=1 and len(posDict[word])=1e10, 
        then we use binary search: O(MlogN). if the lengths are similar, we use one pass O(M+N)
        """
