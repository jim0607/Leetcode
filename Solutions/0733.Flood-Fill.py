        ...
        def dfs(curr_i, curr_j):
            ...
            for delta_i, delta_j in [(1,0), (0,1), (-1,0), (0,-1)]:
                ...
                if 0 <= next_i < len(image) and 0 <= next_j < len(image[0]):
                    if image[next_i][next_j] == color_should_change:
                        dfs(next_i, next_j)
                       
        color_should_change = image[sr][sc]     # we should only change cells with original color
        if color_should_change != newColor:
            dfs(sr, sc)
        return image
