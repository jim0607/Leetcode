...
"""
finding the busiest people is find the largest number in the presum list
"""
def find_busiest_period(data):
    curr_num = data[0][1] if data[0][2] == 1 else -data[0][1]
    max_num = curr_num
    res = data[0][0]
    i = 1
    while i < len(data):
        temp_num = curr_num
        while i < len(data) and data[i][0] == data[i-1][0]:  # merge the data that has the same time
            temp_num = temp_num + data[i][1] if data[i][2] == 1 else temp_num - data[i][1]
            i += 1
        curr_num = temp_num
