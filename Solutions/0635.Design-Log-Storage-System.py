...
class LogSystem:
    ...
    def put(self, id: int, timestamp: str) -> None:
        self.log.append((timestamp, id))

    def retrieve(self, start: str, end: str, gra: str) -> List[int]:
        idx = self.index[gra]
        res = []
        for time, id in self.log:
            if start[:idx] <= time[:idx] <= end[:idx]:
                res.append(id)
        return res
...
Or in java, one can use TreeMap, the TreeMap is maintained internally as a Red-Black(balanced) tree.
Using a balanced tree structure, one might be able to realize logN for retrieve, 
but in expense, put method will cost logN too.
"""
