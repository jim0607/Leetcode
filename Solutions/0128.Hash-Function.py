...
A good hash function can avoid collision as less as possible. A widely used hash function algorithm is using a magic number 33, 
...
"""
取模过程要使用同余定理： (a op b) % MOD = ( ((a % MOD) op (b % MOD)) ) % MOD
"""
class Solution:
    def hashCode(self, s, HASH_SIZE):
        """
        given a string, return the hash code of the string
        """
        hash_code = 0
        for ch in s:
            hash_code = ( hash_code * 33 + ord(ch) ) % HASH_SIZE
