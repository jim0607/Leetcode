...
class Solution:
    def compareVersion(self, version1: str, version2: str) -> int:
        ...
        for i in range(min(len(v1), len(v2))):
            n1 = 0
            for digit in v1[i]:
                n1 = 10 * n1 + ord(digit) - ord("0")
                
            n2 = 0
            for digit in v2[i]:
                n2 = 10 * n2 + ord(digit) - ord("0")
        ...
        if i != len(v1) - 1 and any(digit != "0" for s in v1[i+1:] for digit in s):
            return 1
        if i != len(v2) - 1 and any(digit != "0" for s in v2[i+1:] for digit in s):
            return -1
