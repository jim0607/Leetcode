...
class Solution:
    def findRotateSteps(self, ring: str, key: str) -> int:
        def dfs(curr_ring, curr_idx):
            if curr_idx == len(key) - 1:
                return 0
            ...
            min_step = float("inf")
            for i in range(len(curr_ring)):
                if curr_ring[i] == key[curr_idx+1]:
                    this_step = min(i, len(curr_ring) - i)
                    next_ring = curr_ring[i:] + curr_ring[:i]
                    min_step = min(min_step, this_step + 1 + dfs(next_ring, curr_idx+1))
                 
            memo[(curr_ring, curr_idx)] = min_step
            return min_step        
