...
"""
similar with 394. Coins in a Line. 
dp[i] = the max one can win with [i:] stones left
...
class Solution:
    def stoneGameIII(self, stones: List[int]) -> str:
        ...
        dp[n-1] = stones[n-1]
        dp[n-2] = max(stones[n-2] - stones[n-1], stones[n-2] + stones[n-1])
        if n >= 3:
            dp[n-3] = max(stones[n-3] - dp[n-2], stones[n-3] + stones[n-2] - stones[n-1], stones[n-3] + stones[n-2] + stones[n-1])
