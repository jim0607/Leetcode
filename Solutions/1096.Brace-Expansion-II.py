...
"""
Use stack to store calculated results.  Maintain two lists:
1. the previous list before ",";
2. the current list that is still growing.
When seeing an "alphabet", grow the second list by corss multiplying. So that [a,c]*b will become ["ab", "cb"];
...
class Solution:
    def braceExpansionII(self, expression: str) -> List[str]:
        ...
        for ch in expression:
            if ch.isalpha():
                curr = [char + ch for char in curr or [""]]     # or [""] is in case the curr is empty
            elif ch == "{":
                st.append(prev)
                st.append(curr)
                prev, curr = [], []
                ...
                top = st.pop()
                curr = [t + char for char in prev + curr for t in top or [""]]
                prev = st.pop()
            elif ch == ",":
                prev += curr
                curr = []
        return sorted(set(prev + curr))
