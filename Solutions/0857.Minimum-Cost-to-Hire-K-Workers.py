...
So generally, a team cost is w/q * ∑qi where w/q is the maximum wage/quality ratio in that team.
...
class Solution:
    def mincostToHireWorkers(self, quality: List[int], wage: List[int], K: int) -> float:
        ...
        min_cost = float("inf")
        total_qlt = 0
        hq = []         # max heap to store 之前所有的quality
        for ratio, qlt in workers:
            heappush(hq, -qlt)
            total_qlt += qlt
            if len(hq) > K:
                total_qlt -= -heappop(hq)
            if len(hq) == K:
                min_cost = min(min_cost, total_qlt * ratio)
