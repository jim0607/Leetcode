...
class Solution:
    def canCompleteCircuit(self, gas: List[int], cost: List[int]) -> int:
        ...
        for i, (g, c) in enumerate(zip(gas, cost)):
            gas_left += g - c
            if gas_left < 0:        # if gas_left is negative, then we cannot start from the last possible_station, we need to start at the next possilbe_station
                possible_station = i + 1
                gas_left = 0
                
        return possible_station
